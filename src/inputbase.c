...
void poll_input(struct inputstate_t* state, int* running_condition, int* winw, int* winh, SDL_Window* window)
{
    ...
    // Determine if cursor should be locked right now
    bool shouldLock = false;
    
    if (*gEditorActive) {
        // Editor mode: lock only when right-click dragging in the 3D viewport
        shouldLock = (state->mbutton_right && !ImGui_WantCaptureMouse() || gEditorGui_ViewportCaptured);
    } else {
        // Game mode: lock unless console is open
        shouldLock = !gConsole->visible;
    }

    // Apply the lock state
    if (shouldLock != gCursorLocked) {
        gCursorLocked = shouldLock;
        SDL_SetWindowRelativeMouseMode(window, gCursorLocked);
        
        if (gCursorLocked) {
            // Just locked — warp to center to avoid a jump
    ...
    if (gCursorLocked) {
        ...
        // Cursor unlocked — get absolute position, zero relative ??? no fuck off
        SDL_MouseButtonFlags absmbuttons = SDL_GetMouseState(&state->mx, &state->my);
   
        
        state->mbutton_left  = (absmbuttons & SDL_BUTTON_LMASK) != 0;
        state->mbutton_middle = (absmbuttons & SDL_BUTTON_MMASK) != 0;
        state->mbutton_right = (absmbuttons & SDL_BUTTON_RMASK) != 0;
    }
    ...
    // Toggle editor
    state->FLAG_ToggleEditor = state->kCurrent[SDL_SCANCODE_SEMICOLON] && 
                               !state->kPrevious[SDL_SCANCODE_SEMICOLON];
